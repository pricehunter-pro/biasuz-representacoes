...
async function boot(){if(!sb){statusEl.textContent="Supabase não configurado.";return}const {data:{session}}=await sb.auth.getSession();if(session){const {data:{user}}=await sb.auth.getUser();if(user?.user_metadata?.must_change_password){location.href="./reset-password.html?mode=change";return}if(user?.app_metadata?.role!=="admin"){statusEl.textContent="Acesso administrativo não autorizado.";await sb.auth.signOut();showApp(false);return}showApp(true);await refreshAll()}else showApp(false)}
