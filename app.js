...
function waLink(brand){const n=(cfg.whatsappNumber||"").replace(/\D/g,"");const msg=encodeURIComponent("Olá, sou lojista e tenho interesse comercial em "+brand+" através da Biasuz Representações.");return n?"https://wa.me/"+n+"?text="+msg:"#contato"}
function renderBrands(){
 const grid=document.getElementById("brandsGrid");
 grid.innerHTML=brands.filter(b=>active==="Todas"||b.segments.includes(active)).map(b=>
 '<article class="brand-card"><div><h3>'+esc(b.name)+'</h3><p>'+esc(b.segments.join(" • "))+'</p></div><div class="brand-actions">'+
 '<a href="./brand.html?slug='+encodeURIComponent(b.slug)+'">Catálogo</a>'+
 '<a href="'+esc(b.url)+'" target="_blank" rel="noopener">Site oficial</a>'+
 '<a class="primary" href="'+waLink(b.name)+'">Tenho interesse</a></div></article>'
 ).join("");
...
async function loadBrands(){
 ...
 const {data,error}=await sb.from("representadas").select("name,slug,official_url,segments,active").eq("active",true).order("name");
 if(!error&&data?.length) brands=data.map(b=>({name:b.name,slug:b.slug,url:b.official_url,segments:b.segments||[]}));
 renderBrands();
}
...
document.getElementById("leadForm").addEventListener("submit",async e=>{
 ...
 try{
  ...
  else{const n=(cfg.whatsappNumber||"").replace(/\D/g,"");if(n)window.open("https://wa.me/"+n+"?text="+encodeURIComponent("Novo contato Biasuz\nNome: "+d.name+"\nEmpresa: "+d.company+"\nUF: "+d.state+"\nSegmento: "+d.segment+"\nMarca: "+(d.brand||"não informada")),"_blank");else throw new Error("Canal de atendimento ainda não configurado.")}
  status.className="form-status ok";status.textContent="Solicitação enviada. Em breve entraremos em contato.";form.reset()
